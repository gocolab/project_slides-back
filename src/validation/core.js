...
export async function inspectSlide(page, fileName, slidesDir) {
  ...
  const inspection = await page.evaluate(
    ({ framePx, textSelector, tolerancePx }) => {
      ...
      const hasTailwind = Array.from(document.querySelectorAll('head script[src]'))
        .some((s) => s.src.includes('tailwindcss.com'));
      if (!hasTailwind) {
        critical.push({
          code: 'missing-tailwind-cdn',
          message: 'Slide is missing the required Tailwind Play CDN script: <script src="https://cdn.tailwindcss.com"></script>',
          element: 'head',
        });
      }
