...
test('bundled design styles include Tailwind implementation hints', () => {
  const styles = listDesignStyles();

  // Check a sample of styles for Tailwind hints in layout or fonts
  const stylesWithHints = styles.filter((s) => {
    const layoutMatch = s.layout.some((l) => l.toLowerCase().includes('tailwind'));
    const fontMatch = s.fonts && s.fonts.some((f) => f.toLowerCase().includes('tailwind'));
    return layoutMatch || fontMatch;
  });

  // At least the majority of common styles should have been updated
  assert.ok(stylesWithHints.length > 30, `Expected most styles to have Tailwind hints, but only found ${stylesWithHints.length}`);

  // Specifically check Glassmorphism (01)
  const glass = styles.find((s) => s.id === 'glassmorphism');
  assert.ok(glass.layout.some((l) => l.includes('Tailwind: `bg-white/20 backdrop-blur-[12pt]`')), 'Style 01 should have specific Tailwind hint');
});
