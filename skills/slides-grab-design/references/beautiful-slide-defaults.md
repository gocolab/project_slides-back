...
If the style direction is still open, gather visual references or a mood board first. Define the core tokens early using a consistent **Tailwind color palette**: `background`, `surface`, `primary text`, `muted text`, `accent`, plus typography roles.
...
- Would this still feel premium without extra chrome (shadows, cards)?
